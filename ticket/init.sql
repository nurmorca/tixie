...
CREATE TABLE IF NOT EXISTS TICKET (
    ...
    TI_EVENT_ID INT REFERENCES events(ev_id),
    TI_SEAT_ID INT REFERENCES seat(se_id),
    TI_PRICE DECIMAL(10, 2) NOT NULL,
    TI_STATUS VARCHAR(20) DEFAULT 'available',
    TI_CREATED_AT TIMESTAMP NOT NULL DEFAULT NOW(),
    UNIQUE(TI_EVENT_ID, TI_SEAT_ID)
);

ALTER TABLE TICKET ADD CONSTRAINT TI_STATUS CHECK (TI_STATUS IN ('AVAILABLE', 'SOLD', 'RESERVED'));
...
INSERT INTO ticket (TI_EVENT_ID, TI_SEAT_ID, TI_STATUS, TI_PRICE)
SELECT 
    1,
    se_id,
    'AVAILABLE',
